"""CLI: fetch sentiment stream items for a market and append them to storage.

Usage::

    python -m detector_fin.fetch_sentiment --market CN --since 2026-06-01

Loads the universe, selects the market's sentiment adapter, fetches public
discussion items for every instrument in that market, and appends the RawItem
records to the ``raw_items`` dataset of a ParquetStore.
"""
...
def build_adapter(market_id: str):
    """Return the sentiment adapter instance registered for ``market_id``."""
    from .fetcher.sentiment import SENTIMENT_ADAPTERS

    mid = market_id.strip().upper()
    try:
        return SENTIMENT_ADAPTERS[mid]()
    except KeyError:
        raise SystemExit(f"no sentiment adapter for market {mid!r}")
...
def run(
    ...
    *,
    ...
    """Fetch sentiment items for one market and append them to the store."""
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="python -m detector_fin.fetch_sentiment",
        description="Fetch sentiment stream items for a market and append to storage.",
    )
    ...
    parser.add_argument(
        ...
        help="fetch items on/after this date (YYYY-MM-DD, UTC)",
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"Fetched and stored {len(items)} sentiment items for market "
